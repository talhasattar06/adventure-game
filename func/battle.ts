...
export async function battle(player: Player, enemies: string[], enemyMaxHealth: number, weapons: string[], theme: string) {
    let fight = true;

    while (fight) {
        let enemyHealth = randomNum(30, enemyMaxHealth);
        let enemy: string = enemies[randomNum(0, enemies.length)];
        ...
        while (enemyHealth > 0) {
            ...
            switch (ask.option) {
                case "Attack":
                    ...
                    let damageTaken: number = randomNum(1, new Enemy().enemyAttack);
